...
def env_3(agent, get_value):
    agent.learn(1e3)
    assert  pytest.approx(GAMMA, rel=EPS) == get_value(agent, np.array([0])) # Comment gérer le gamma pour l'imposer sur l'agent?
